  ...
  return (
    <div className="text-center animate-fade-in">
      <div className="flex justify-center items-center gap-2 mb-4">
        ...
        <h2 className="text-2xl md:text-3xl font-bold text-slate-800 dark:text-white">{t('category_title')}</h2>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {SERVICE_CATEGORIES.map((category) => (
          ...
          >
            ...
            <h3 className="font-semibold text-lg text-slate-800 dark:text-white mb-1">{t(category.name.toLowerCase())}</h3>
            <p className="text-sm text-slate-500 dark:text-slate-400">{t(`${category.name.toLowerCase()}_desc`)}</p>
          </button>
        ))}
      </div>
       <div className="mt-8 text-center">
        <button onClick={onBack} className="text-sm text-sky-600 dark:text-sky-400 hover:underline">&larr; {t('change_location')}</button>
      </div>
    </div>
  );
};
